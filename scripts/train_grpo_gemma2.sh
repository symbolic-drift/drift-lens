#!/usr/bin/env bash
#
# GRPO fine-tuning of Gemma-2-2B-IT on the SymbolicDrift training data.
# See scripts/train_grpo_qwen3.sh for the full configuration explanation.
set -euo pipefail
set -x

PROJECT_NAME=${PROJECT_NAME:-Symbolic_GRPO-gemma2}
...
TRAIN_FILE=${TRAIN_FILE:-"${DATA_DIR}/gemma2_train.parquet"}
VAL_FILE=${VAL_FILE:-"${DATA_DIR}/gemma2_test.parquet"}
